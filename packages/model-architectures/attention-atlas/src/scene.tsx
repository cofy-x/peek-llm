...
function GraphicsFallback({ onRetry }: { onRetry?: () => void }) {
  return (
    ...
    >
      ...
      <span>The component explorer, computation steps, and paper notes are still available.</span>
      <table style={{ borderCollapse: "collapse", fontSize: 12, maxWidth: 560 }}>
        <caption style={{ marginBottom: 10 }}>Architecture at a glance</caption>
        <thead><tr><th scope="col">Model</th><th scope="col" style={{ paddingInline: 14 }}>Encoder → decoder</th><th scope="col">Computation</th></tr></thead>
        <tbody>
          <tr><th scope="row">Transformer</th><td style={{ padding: 8 }}>6 bidirectional → 6 causal</td><td>Dense attention and FFN</td></tr>
          <tr><th scope="row">DeepSeek</th><td style={{ padding: 8 }}>20 causal → 20 causal</td><td>Sparse attention, MoE, shared global KV</td></tr>
        </tbody>
      </table>
...
export function AtlasScene(props: AtlasSceneProps) {
  ...
  const supported = useMemo(() => supportsWebGL2(), [generation]);
  const onLost = useCallback(() => setLost(true), []);
  const retry = useCallback(() => {
    setLost(false);
    setGeneration((value) => value + 1);
  }, []);
  if (lost || !supported) return <GraphicsFallback onRetry={retry} />;
  return (

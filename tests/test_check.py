...
class SelfContainedContractTests(unittest.TestCase):
    ...
    def test_allows_only_registered_source_module(self) -> None:
        source = "../../src/entries/attention-atlas.ts"
        html = BASE.format(head='<meta name="peek:mode" content="source">', body=f'<script type="module" src="{source}"></script>', script="", tail="")
        self.assertEqual(peek_check.validate_text(html, source), [])
        self.assertIn("source mode requires an explicit registry entry", peek_check.validate_text(html))
        self.assertIn("external script src is forbidden", peek_check.validate_text(html.replace(source, "https://cdn.example/app.js"), source))

    def test_source_registration_does_not_relax_network_or_styles(self) -> None:
        source = "../../src/entries/attention-atlas.ts"
        html = BASE.format(head='<meta name="peek:mode" content="source"><link rel="stylesheet" href="https://cdn.example/x.css">', body=f'<script type="module" src="{source}"></script>', script='fetch("https://example.com/data")', tail="")
        errors = peek_check.validate_text(html, source)
        self.assertIn("linked resources are forbidden; inline them", errors)
        self.assertIn("runtime fetch requires declared optional network access", errors)

    def test_registered_source_requires_exactly_one_matching_module(self) -> None:
        html = BASE.format(head='<meta name="peek:mode" content="source">', body="", script="", tail="")
        self.assertIn("registered source entry requires one matching module and peek:mode=source", peek_check.validate_text(html, "../../src/entries/example.ts"))
...
class PortalEntryTests(unittest.TestCase):
    def run_check(self, html: str | None) -> tuple[int, str, str]:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(peek_check, "ROOT", root), contextlib.redirect_stdout(out), contextlib.redirect_stderr(err):
                status = peek_check.main()
            return status, out.getvalue(), err.getvalue()

    def test_checks_portal_without_template_or_reference_directories(self) -> None:
        html = BASE.format(head='<meta name="peek:mode" content="source">', body='<script type="module" src="./src/main.tsx"></script>', script="", tail="")
        status, out, err = self.run_check(html)
        self.assertEqual(status, 0)
        self.assertIn("ok: apps/portal/index.html", out)
        self.assertEqual(err, "")

    def test_missing_portal_fails_with_actionable_error(self) -> None:
        status, out, err = self.run_check(None)
        self.assertEqual(status, 1)
        self.assertEqual(out, "")
        self.assertIn("apps/portal/index.html: cannot read Portal entry", err)

    def test_invalid_portal_does_not_pass(self) -> None:
        html = BASE.format(head='<meta name="peek:mode" content="source">', body='<script type="module" src="./src/wrong.tsx"></script>', script="", tail="")
        status, out, err = self.run_check(html)
        self.assertEqual(status, 1)
        self.assertEqual(out, "")
        self.assertIn("one matching module", err)

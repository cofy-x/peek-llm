#!/usr/bin/env python3
"""Validate the Portal HTML entry using the standard library."""
...
class ExhibitParser(HTMLParser):
    def __init__(self, source_script: str | None = None) -> None:
        super().__init__(convert_charrefs=True)
        self.errors: list[str] = []
        self.source_script = source_script
        self.source_script_count = 0
        self.authoring_mode: str | None = None
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "html":
            ...
            if values.get("name", "").lower() == "peek:mode":
                self.authoring_mode = values.get("content", "")
        ...
        if tag == "script":
            ...
            if "src" in values:
                if self.source_script and values["src"] == self.source_script and values.get("type") == "module":
                    self.source_script_count += 1
                else:
...
def validate_text(text: str, source_script: str | None = None) -> list[str]:
    parser = ExhibitParser(source_script)
    parser.feed(text)
    errors = list(parser.errors)
    if source_script and (parser.source_script_count != 1 or parser.authoring_mode != "source"):
        errors.append("registered source entry requires one matching module and peek:mode=source")
    if parser.authoring_mode and not source_script:
        errors.append("source mode requires an explicit registry entry")
...
def main() -> int:
    ...
    entries: dict[Path, str] = {portal / "index.html": "./src/main.tsx"}
    ...
    for path in entries:
        try:
            ...
            failures += 1
            print(f"{path.relative_to(ROOT)}: cannot read Portal entry: {error}", file=sys.stderr)
            continue
        errors = validate_text(text, entries[path])
        if errors:
            failures += 1
